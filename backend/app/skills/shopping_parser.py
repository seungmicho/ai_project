...
# OpenAI 클라이언트는 lazy 생성 — 키가 없어도 import 시점에 죽지 않게.
# 함수 호출 시 키가 비어있으면 명시적인 에러 메시지를 반환합니다.
_openai_client: "OpenAI | None" = None


def _get_openai_client() -> "OpenAI | None":
    """OpenAI 클라이언트를 필요할 때만 생성합니다 (lazy init)."""
    global _openai_client
    if _openai_client is not None:
        return _openai_client
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        logger.warning("OPENAI_API_KEY 환경변수가 비어 있습니다 — 쇼핑 파서 비활성화.")
        return None
    _openai_client = OpenAI(api_key=api_key)
    return _openai_client


# backend/app/models/clothing.py의 ClothingItem ORM 컬럼과 1:1 매핑되는 출력 스키마.
# - name, category, color, tags, image_url: backend 표준 필드
# - fit_info: fashion-backend 확장 필드 (이미지 기반 핏 판단)
# tags는 쉼표 구분 문자열 (예: "캐주얼,봄") — backend 컨벤션 준수
    ...
    "purchased_items": [
        {
            ...
            "tags": "쉼표로 구분한 분위기/시즌 태그 (예: '캐주얼,봄', '포멀,겨울')",
            "image_url": "스크린샷 안에 상품 이미지 URL이 보이면 그 URL, 안 보이면 빈 문자열",
...
def parse_shopping_screenshot(image_file) -> dict:
    ...
    if client is None:
        return {"status": "error", "message": "OPENAI_API_KEY가 설정되지 않았습니다. .env 파일에 키를 추가해 주세요."}
...
def parse_shopping_history(text_input: str) -> dict:
    ...
    tags는 쉼표 구분 문자열로 분위기/시즌을 채워줘 (예: '캐주얼,봄'). 텍스트에 단서가 없으면 빈 문자열.
    image_url은 텍스트에 URL이 명시돼 있을 때만 채우고, 아니면 빈 문자열.
    마크다운 기호(```) 없이 순수한 JSON만 출력해.

    출력 형식:
    {"purchased_items": [{"name": "...", "category": "...", "color": "...", "tags": "...", "image_url": "...", "fit_info": "..."}]}
    """
